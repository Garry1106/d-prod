...
const SecureField = ({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) => {
  ...
  return (
    <motion.div 
      className="mb-4 p-4 bg-white rounded-xl border border-gray-100 hover:border-[#EB6C33]/30 shadow-sm hover:shadow transition-all" 
      whileHover={{ y: -2 }}
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <button 
          ...
          className="text-xs px-3 py-1 rounded-full bg-[#EB6C33]/10 text-[#EB6C33] hover:bg-[#EB6C33]/20 transition-colors"
        >
          {revealed ? "Hide" : "Show"}
        </button>
      </div>
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center">
          {value ? (
            <div className="flex items-center bg-[#EB6C33]/10 text-[#EB6C33] px-3 py-1 rounded-full">
...
const UsageProgress = ({ label, used, total, icon }: { label: string; used: number; total: number; icon: React.ReactNode }) => {
  ...
  return (
    <motion.div 
      className="mb-6 p-4 rounded-xl border border-gray-100 hover:border-[#EB6C33]/30 shadow-sm hover:shadow bg-white transition-all"
      whileHover={{ y: -2 }}
    >
  ...
  return (
    <motion.div 
      className={`p-4 rounded-xl shadow-sm border ${
        isLow ? 'bg-amber-50 border-amber-200' : 'bg-[#EB6C33]/5 border-[#EB6C33]/20'
      }`}
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className={`px-3 py-1 rounded-full text-sm font-medium ${
          isLow ? 'bg-amber-100 text-amber-600' : 'bg-[#EB6C33]/10 text-[#EB6C33]'
        }`}>
      ...
      <div className="mt-4">
        <div className="w-full bg-gray-200 rounded-full h-1.5">
          <div 
            className={`h-1.5 rounded-full ${isLow ? 'bg-amber-500' : 'bg-[#EB6C33]'}`}
      ...
      <div className="mt-4">
        <button className={`w-full py-2 rounded-lg text-white font-medium text-sm ${
          isLow ? 'bg-amber-500 hover:bg-amber-600' : 'bg-[#EB6C33] hover:bg-[#EB6C33]/90'
        } transition-colors`}>
          Renew Subscription
        </button>
      </div>
...
export default function ProfilePage() {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-white to-gray-50 p-6 ${raleway.className} ${inter.variable}`}>
            ...
            >
              ...
              <motion.div variants={itemVariants} className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 bg-white p-6 rounded-2xl shadow-md border border-gray-100">
                <div className="flex items-center mb-4 sm:mb-0">
                  <Avatar className="h-16 w-16 mr-4 border-2 border-[#EB6C33] shadow-lg">
                    <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${user?.businessName}`} alt={user?.businessName} />
                    <AvatarFallback className="bg-[#EB6C33] text-white text-xl">
                      {user?.businessName?.split(' ').map(word => word[0]).join('') || 'TS'}
                    </AvatarFallback>
                  </Avatar>
                  
                  <div>
                    <h1 className="text-2xl font-bold text-gray-800">{user?.businessName || ''}</h1>
                    <div className="flex items-center mt-1">
                      <Badge className="bg-[#EB6C33]/10 text-[#EB6C33] hover:bg-[#EB6C33]/20 border-none">
                        {user?.selectOption}
                      </Badge>
                ...
                <div className="w-full sm:w-auto">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    <Badge className="bg-[#EB6C33] text-white hover:bg-[#EB6C33]/90">
                ...
                <motion.div variants={itemVariants} className="md:col-span-1 space-y-6">
                  ...
                  <motion.div variants={cardVariants}>
                    <Card className="border-gray-200 shadow-sm overflow-hidden">
                      <CardHeader className="bg-gradient-to-r from-[#EB6C33]/10 to-transparent pb-3">
                        <div className="flex justify-between items-center">
                          <CardTitle className="text-lg font-bold text-gray-800">API Credentials</CardTitle>
                          <Lock className="h-5 w-5 text-[#EB6C33]" />
                        </div>
                      </CardHeader>
                      <CardContent className="pt-4">
                        <SecureField label="WABA ID" value={user?.waba_id || ''} icon={<Key size={18} />} />
                        <Field label="Phone Number" value={user?.displayPhoneNumber || ''} icon={<Phone size={18} />} />
                        <SecureField label="Phone ID" value={user?.phoneNumberId || ''} icon={<Key size={18} />} />
                        ...
                        <div className="space-y-2 max-h-[250px] overflow-y-auto pr-2">
                          {features.map((feature, index) => (
                            <div key={index} className={`flex items-center p-2 rounded-lg ${
                              ...
                              {feature.enabled ? (
                                <CheckCircle className="h-4 w-4 text-[#EB6C33] mr-2 flex-shrink-0" />
                              ) : (
                ...
                <motion.div variants={itemVariants} className="md:col-span-2">
                  <Tabs defaultValue="usage" className="w-full">
                    <TabsList className="w-full bg-white p-1 rounded-xl mb-6 border border-gray-200">
                      <TabsTrigger value="usage" className="flex-1 rounded-lg data-[state=active]:bg-[#EB6C33] data-[state=active]:text-white">
                        <BarChart className="h-4 w-4 mr-2" />
                        Usage
                      </TabsTrigger>
                      <TabsTrigger value="account" className="flex-1 rounded-lg data-[state=active]:bg-[#EB6C33] data-[state=active]:text-white">
                        <User className="h-4 w-4 mr-2" />
                        Account
                      </TabsTrigger>
                      <TabsTrigger value="settings" className="flex-1 rounded-lg data-[state=active]:bg-[#EB6C33] data-[state=active]:text-white">
                    ...
                    <TabsContent value="usage" className="mt-0">
                      <Card className="border-gray-200 shadow-sm">
                        <CardHeader className="bg-gradient-to-r from-[#EB6C33]/10 to-transparent">
                          <div className="flex justify-between items-center">
                        ...
                        <CardContent className="pt-6">
                          ...
                          <div className="mt-6 p-4 rounded-xl bg-gray-50 border border-gray-200">
                            <div className="flex items-start">
                              <Info className="h-5 w-5 text-[#EB6C33] mr-3 mt-0.5" />
                              <div>
                    ...
                    <TabsContent value="account" className="mt-0">
                      <Card className="border-gray-200 shadow-sm">
                        ...
                        <CardContent className="pt-6">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Field label="Business Name" value={user?.businessName || ''} icon={<Building size={18} />} />
                            <Field label="Account Type" value={user?.selectOption || ''} icon={<Database size={18} />} />
                            <Field label="Country" value={user?.country || ''} icon={<Globe size={18} />} />
                            <Field label="Currency" value={user?.currency || ''} icon={<CreditCard size={18} />} />
                            <Field label="Clerk ID" value={user?.clerkId || ''} icon={<User size={18} />} />
                            <Field label="Subscription Level" value={user?.subscriptionLevel || ''} icon={<Database size={18} />} />
                          </div>
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="p-4 rounded-xl border border-gray-200 bg-white">
                              ...
                              <div className="flex items-center justify-between p-4 rounded-lg border border-[#EB6C33]/20 bg-[#EB6C33]/5">
                                <div>
                                  <p className="font-semibold text-gray-800">Current Plan: <span className="text-[#EB6C33]">{user?.subscriptionLevel}</span></p>
                                  <p className="text-sm text-gray-600 mt-1">
                            ...
                            <div className="p-4 rounded-xl border border-gray-200 bg-white">
                              ...
                              <div className="space-y-3">
                                <button className="w-full flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:border-[#EB6C33]/30 bg-gray-50 hover:bg-gray-100 transition-all">
                                  <div className="flex items-center">
                                    <Lock className="h-5 w-5 text-[#EB6C33] mr-2" />
                                ...
                                <button className="w-full flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:border-[#EB6C33]/30 bg-gray-50 hover:bg-gray-100 transition-all">
                                  <div className="flex items-center">
                                    <Key className="h-5 w-5 text-[#EB6C33] mr-2" />
                                ...
                                <button className="w-full flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:border-[#EB6C33]/30 bg-gray-50 hover:bg-gray-100 transition-all">
                                  <div className="flex items-center">
                                    <Settings className="h-5 w-5 text-[#EB6C33] mr-2" />

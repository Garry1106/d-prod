...
const ProductPage = () => {
  ...
  const handlePricingClick = (id: number | string) => {
    if (typeof id === 'number') {
      setSelectedPlan(id);
    } else {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-7xl mx-auto px-6">
              {pricingOptions.map((plan) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.4, delay: 0.15 * pricingOptions.indexOf(plan) }}
                  className={`bg-white rounded-xl shadow-xl overflow-hidden border-2 transform hover:scale-105 transition-all duration-300 ${
                    selectedPlan === plan.id ? 'border-[#EB6C33]' : 'border-gray-200'
                  }`}
                >
                      ...
                      {featuresList.map((feature, index) => (
                        <li key={index} className="flex items-center">
                          {plan.features.includes(feature) ? (
                            <svg
                              className="w-5 h-5 text-[#EB6C33] mr-2 flex-shrink-0"

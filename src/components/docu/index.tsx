...
import { FiHome, FiHelpCircle, FiCode } from 'react-icons/fi';
  ...
  return (
    <div className="min-h-screen  flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl px-4 mt-10">
        {[
          { name: 'Getting Started', icon: <FiHome />, href: '#' },
          { name: 'Installation', icon: <FiCode />, href: '#' },
          { name: 'API Reference', icon: <FiCode />, href: '#' },
          { name: 'Help', icon: <FiHelpCircle />, href: '#' },
        ].map((item, idx) => (
          <a
            key={idx}
            href={item.href}
            className="flex items-center bg-[#EB6C33] text-white p-4 rounded-lg shadow-md transition hover:shadow-xl"
          >
          ...
          {[
            {
              ...
              description:
                'Thanks to premade assets, you can modify all colors and text, such as the Action Color.',
            },
            {
              title: 'Components',

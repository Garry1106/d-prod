...
import { Phone, MoreVertical, Smile, Send } from "lucide-react"; // Assuming you're using Lucide icons
...
const PreviewPane: React.FC<PreviewPaneProps> = ({
  ...
  return (
    <div className="relative mx-auto border-4 border-black rounded-[40px] w-[300px] h-[550px] overflow-hidden bg-white shadow-gray-400 shadow-lg">
      {/* Notch */}
        ...
        <div className="flex-1 px-3 py-2 space-y-2 overflow-y-auto scrollbar-hide">
          ...
          <div className="flex items-start gap-2">
            <div className="bg-white p-3 rounded-lg max-w-[70%] shadow-sm border border-gray-200">
              {/* Header Section */}
              {activeMediaTab === "text" && headerText && (
                <p className="text-gray-800 text-sm font-semibold mb-2">
                  {headerText}
                </p>
              )}
              {activeMediaTab === "image" && mediaFile && (
                <div
                  className="w-full aspect-[1.91/1] rounded-lg mb-2 overflow-hidden"
                  style={{
              ...
              {activeMediaTab === "video" && mediaFile && (
                <video
              ...
              {activeMediaTab === "document" && mediaFile && (
                  ...
                  {buttons.map((button, index) => (
                    ...
                    >
                      ...
                      {button.type === "quick_reply" && (
                        <span className="text-gray-500 text-sm">Quick Reply</span>
                      )}
                    </div>
                  ))}
                </div>

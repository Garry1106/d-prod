...
async function GET(req) {
    try {
        ...
        const apiUrl = new URL(`https://graph.facebook.com/${API_VERSION}/${WABA_ID}/message_templates`);
        ...
        const exactMatch = result.data.find((template) => template.name === exactName);
        if (!exactMatch) {
            return server_1.NextResponse.json({ error: "Template not found" }, { status: 404 });
        }
        return server_1.NextResponse.json({
            data: Object.assign(Object.assign({}, exactMatch), { 
        ...
        return server_1.NextResponse.json({ error: "Failed to fetch template details" }, { status: 500 });
    }
}

...
async function POST(request) {
    try {
        ...
        if (!waId || !businessPhoneNumber) {
            return server_1.NextResponse.json({ success: false, error: "waId and businessPhoneNumber are required." }, { status: 400 });
        }
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to update alert status." }, { status: 500 });
    }
}
